...
@Component({
    selector: 'app-partner-quick-modal',
    templateUrl: './partner-quick-modal.component.html',
    standalone: true,
    imports: [CommonModule, ReactiveFormsModule, ModalComponent, LocalizationPipe]
})
export class PartnerQuickModalComponent {
    ...
    open(initialName: string = ''): void {
        this.form.reset({
            name: initialName,
            type: ClientType.LocalProducer, // Sempre LocalProducer para parceiros
            email: '',
            phone: ''
        });
